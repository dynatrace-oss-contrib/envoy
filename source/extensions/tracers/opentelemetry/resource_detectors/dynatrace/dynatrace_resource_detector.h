 ...
 *
 * Dynatrace can be deployed in multiple ways and flavours, depending on the environment.
 * The available Dynatrace enrichment files vary depending on how it is deployed. E.g. In k8s via the Dynatrace operator.
 *
 * Since the resource detector is not aware how Dynatrace is deployed, the detector attempts to reads all
 * Dynatrace enrichment files. In such cases, reading some of these files may fail
 * but this is expected and does not classify as a problem with the detector. The detector may also not detect
 * any attributes, for example when a Dynatrace deployment is not successfull. In such cases, Envoy will be started
 * with no enrichment but Dynatrace users have the means to find out wich Dynatrace deployment is failing.
 *
 * @see
